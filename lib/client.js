...
.dsh-mp2-trigger{display:flex;align-items:stretch;min-width:0;max-width:220px;height:28px;border:none;border-radius:24px;outline:none;background:transparent;color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px;font-weight:500;cursor:pointer;overflow:hidden;padding:0}
...
.dsh-mp2-option{display:flex;align-items:center;gap:8px;width:100%;min-height:38px;padding:6px 8px;border:none;border-radius:10px;outline:none;background:transparent;color:inherit;text-align:left;cursor:pointer}
...
`
   ...
   */
  function ModelRow({ group, provider, model, detail, selected, busy, favorite, facts, bridged, onToggle, onChoose }) {
    ...
    return h('button', {
      ...
      className: 'dsh-mp2-option' + (selected ? ' dsh-mp2-selected' : ''),
      disabled: busy,
      onClick: onChoose,
    },
      h('span', { className: 'dsh-mp2-optionCopy' },
        h('span', { className: 'dsh-mp2-nameRow' },
          h('span', {
            ...
            style: { '--dsh-mp2-hue': String(providerHue(chip.id)) },
            'aria-hidden': true,
          }, providerInitial(chip.name)),
  ...
  function ModelPicker(props) {
    ...
    // The trigger's tooltip names the supplier too. The same model id is
    // routinely served by several providers, so the name alone does not say
    // where the model runs — and the supplier to name is the *display* one, so
    // a selection on a folded `(modlens vision)` route still reads as its base
    // provider, matching the column the user picked from.
    const modelTip = currentChoice === null ? modelLabel : (
      (providers.find((row) => row.models.some(({ group, model }) =>
        group.id === currentChoice.group.id && model.id === currentChoice.model.id))?.name
    ...
    /** Scroll the list; jsdom and older carriers have no smooth `scrollTo`. */
